...
<body>
	...
	try {
		...
		System.out.println("Invalid pageNumber: " + e.getMessage());
	}
 ...
 %></li>
	...
	<section class="container mt-5">
		...
		<%
		ArrayList<EvaluationDTO> evaluationsList = new EvaluationDAO().getList(lectureDivide, search, pageNumber);
		if (evaluationsList == null || evaluationsList.isEmpty()) {
		%>
		<p class="text-center mt-4">No evaluations available for the
			selected filters.</p>
		<%
		} else {
	...
	<%
	// DAO에서 총 레코드 수를 가져옵니다.
	int totalRecords = new EvaluationDAO().getTotalRecords(lectureDivide, search);
	int totalPages = (int) Math.ceil((double) totalRecords / 10); // 페이지당 10개 표시
	%>
	<ul class="pagination justify-content-center mt-3">
		<!-- Previous Button -->
		<li class="page-item <%=pageNumber <= 1 ? "disabled" : ""%>"><a
			class="page-link"
			href="?pageNumber=<%=pageNumber - 1%>&lectureDivide=<%=lectureDivide%>&search=<%=search%>">
				Previous </a></li>
		...
		<li class="page-item <%=i == pageNumber ? "active" : ""%>"><a
			class="page-link"
			href="?pageNumber=<%=i%>&lectureDivide=<%=lectureDivide%>&search=<%=search%>">
				<%=i%>
		</a></li>
		<%
		}
